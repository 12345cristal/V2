...
class Cita(Base):
    ...
    es_nuevo_nino = Column(Boolean, default=False)
    ...
    estado_id = Column(Integer, ForeignKey("cat_estado_cita.id", ondelete="RESTRICT"))
    es_reposicion = Column(Boolean, default=False)
...
class CitaObservador(Base):
    ...
    terapeuta_id = Column(Integer, ForeignKey("personal.id", ondelete="CASCADE"))
