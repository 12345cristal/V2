...
SEXO_ENUM = ("M", "F", "O")
ESTADO_NINO_ENUM = ("ACTIVO", "BAJA_TEMPORAL", "INACTIVO")
NIVEL_COMPRENSION_ENUM = ("ALTO", "MEDIO", "BAJO")
...
class Nino(Base):
    ...
    sexo = Column(Enum(*SEXO_ENUM, name="sexo_enum"), nullable=False)
    curp = Column(String(18))
    tutor_principal_id = Column(
        Integer,
    ...
    estado = Column(
        Enum(*ESTADO_NINO_ENUM, name="estado_nino_enum"),
        nullable=False,
        default="ACTIVO",
    )
    ...
    diagnostico = relationship(
        "NinoDiagnostico", back_populates="nino", uselist=False
    )
    alergias = relationship("NinoAlergias", back_populates="nino", uselist=False)
    medicamentos_actuales = relationship(
        "NinoMedicamentosActuales", back_populates="nino"
    )
    escolar = relationship("NinoEscolar", back_populates="nino", uselist=False)
    contactos_emergencia = relationship(
        "NinoContactoEmergencia", back_populates="nino"
    )
    info_emocional = relationship(
        "NinoInfoEmocional", back_populates="nino", uselist=False
    )
...
class NinoMedicamentosActuales(Base):
    ...
    sin_medicamentos = Column(Integer, nullable=False, default=0)
...
class NinoEscolar(Base):
    ...
    asiste_escuela = Column(Integer, nullable=False, default=1)
...
class NinoInfoEmocional(Base):
    ...
    nivel_comprension = Column(
        Enum(*NIVEL_COMPRENSION_ENUM, name="nivel_comprension_enum"),
        nullable=False,
        default="MEDIO",
    )
