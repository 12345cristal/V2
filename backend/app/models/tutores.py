...
class Tutor(Base):
    ...
    direccion = relationship("TutorDireccion", back_populates="tutor", uselist=False)
    ninos_principales = relationship("Nino", back_populates="tutor_principal")
...
class TutorDireccion(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    tutor_id = Column(Integer, ForeignKey("tutores.id", ondelete="CASCADE", onupdate="CASCADE"), nullable=False)
